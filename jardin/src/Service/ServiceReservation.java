...
public class ServiceReservation {
   ...
   public List<Reservation> afficherres(){
    ...
    return null;
     ...
     public List<String> displaysm()
       ...
       Connection c=DataBase.getInstance().getConnection();
         try {
            PreparedStatement pt =c.prepareStatement("select nom from utilisateur ");
            ResultSet rs= pt.executeQuery();
    ...
    public int getidparent(String nom) {
        ...
        String requete="SELECT id_utilisateur FROM `utilisateur` WHERE nom='"+nom+"' ";
        PreparedStatement pst=c.prepareStatement(requete);
         ResultSet rs=pst.executeQuery();
       
        while(rs.next()){
            
            r = rs.getInt("id_utilisateur");
            return r;
        }    
     } catch (SQLException ex) {
            Logger.getLogger(DataBase.class.getName()).log(Level.SEVERE, null, ex);
 ...
 Connection c=DataBase.getInstance().getConnection();
         try {
            PreparedStatement pt =c.prepareStatement("select nom from enfant where id_utilisateur = ? ");
            pt.setInt(1, id);
